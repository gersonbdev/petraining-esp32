...
ptBasicData initLocalVariables(){
  ...
  a.RealTimeApp[0] = (EEPROM.read(1)<<8) | (EEPROM.read(2));
  a.RealTimeApp[1] = EEPROM.read(3);
  a.RealTimeApp[2] = EEPROM.read(4);
  a.RealTimeApp[3] = EEPROM.read(5);
  a.RealTimeApp[4] = EEPROM.read(6);
  a.RealTimeApp[5] = EEPROM.read(7);
  
  // We get values of pet's age form EEPROM to our calculation variables
  a.PetBirthDay[0] = (EEPROM.read(8)<<8) | (EEPROM.read(9));
  a.PetBirthDay[1] = EEPROM.read(10);
  a.PetBirthDay[2] = EEPROM.read(11);
  a.PetBirthDay[3] = EEPROM.read(12);
  a.PetBirthDay[4] = EEPROM.read(13);
  a.PetBirthDay[5] = EEPROM.read(14);

  // Quantity of food for everyone case
  a.QuantityOfFood[0] = (EEPROM.read(15)<<8) | (EEPROM.read(16)); // Quantity of food if younger than 3 months
  a.QuantityOfFood[1] = (EEPROM.read(17)<<8) | (EEPROM.read(18)); // Quantity of food if between 3 and 6 months
  a.QuantityOfFood[2] = (EEPROM.read(19)<<8) | (EEPROM.read(20)); // Quantity of food if elder than 6 months

  // Food rations for everyone case
  a.FoodRations[0] = EEPROM.read(21); // Food rations if younger than 3 months
  a.FoodRations[1] = EEPROM.read(22); // Food rations if between 3 and 6 months
  a.FoodRations[2] = EEPROM.read(23); // Food rations if elder than 6 months
